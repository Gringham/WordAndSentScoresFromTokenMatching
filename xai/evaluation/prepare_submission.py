...
def align_scores(ck, h):
    ...
    while hyp_pos < len(hyp):
        if hyp[hyp_pos] == ck[ck_pos][1]:
            ...
            # if the assigned scores token is part of the hypothesis token, I have a look at the next tokens in assigned scores
            ...
            # if the hypothesis token is part of the assigned scores token, I have a look at the next tokens in the hypothesis
            int_string = hyp[hyp_pos]
            res.append((ck[ck_pos][0], hyp[hyp_pos]))

            while int_string + hyp[hyp_pos + 1] in ck[ck_pos][1]:
                # As long as multiple words in the hyp are part of one token in the importance scores, I add the words
            ...
            # in all other cases I assume that tokenization has dropped this token and assign it with the median of scores
...
if __name__ == '__main__':
    ...
    path = os.path.join(ROOT_DIR,'xai/output/explanations/0_999_ro_en_test_attributions_bs_xlmr_ensemble')
    invert = True
    lp = 'ro-en'
    ret_key = 'BERTSCORE'
    # check we are matching the structure of the orig file
    test_df = pd.read_csv(os.path.join(ROOT_DIR,'metrics/corpora/pandas_corpora/eval4NLP/eval4nlp_test_' + lp + '.tsv'), delimiter='\t')
